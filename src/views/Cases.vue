<template>
  <div class="cases">
    ...
    <div class="action-bar">
      <button class="add-button" @click="openAddForm">+ 添加案件</button>
      <div class="search-box">
        <input type="text" placeholder="搜索案件..." v-model="searchQuery" />
      ...
      <table>
        ...
        <tbody>
          <tr v-for="caseItem in filteredCases" :key="caseItem.id">
            ...
            <td>{{ caseItem.applicationDate }}</td>
            <td>{{ caseItem.deadlineDate }}</td>
            <td>¥{{ caseItem.paidAmount.toLocaleString() }}</td>
            <td>
              <span :class="['amount-badge', caseItem.owedAmount > 0 ? 'owed' : 'paid']">
                ¥{{ caseItem.owedAmount.toLocaleString() }}
              </span>
            </td>
            <td>
              <div class="action-buttons">
                <button class="edit-button" @click="openEditForm(caseItem)">编辑</button>
                <button class="view-button" @click="handleViewCase(caseItem)">查看</button>
                <button class="delete-button" @click="handleDeleteCase(caseItem)">删除</button>
              </div>
            </td>
          </tr>
        </tbody>
    ...
    <div v-if="showForm" class="form-dialog">
      <div class="form-dialog-content">
        <div class="form-dialog-header">
          <h3>{{ isEditing ? '编辑案件' : '添加案件' }}</h3>
          <button class="close-button" @click="showForm = false">×</button>
        </div>
        <div class="form-dialog-body">
          <form @submit.prevent="submitForm">
            <div class="form-item">
              <label for="name">案件名称 *</label>
              <input type="text" id="name" v-model="formData.name" placeholder="请输入案件名称" required />
            ...
            <div class="form-item">
              ...
              <input type="date" id="applicationDate" v-model="formData.applicationDate" required />
              <span v-if="errors.applicationDate" class="error-message">{{ errors.applicationDate }}</span>
            </div>
            <div class="form-item">
              <label for="deadlineDate">截止日期 *</label>
              <input type="date" id="deadlineDate" v-model="formData.deadlineDate" required />
              <span v-if="errors.deadlineDate" class="error-message">{{ errors.deadlineDate }}</span>
            </div>
            <div class="form-item">
...
// 模拟数据
const cases = ref([
  { id: 1, name: '商标注册 - 阿里云', customer: '阿里巴巴集团', type: '商标', status: '进行中', applicationDate: '2026-01-15', deadlineDate: '2026-04-15', paidAmount: 3000, owedAmount: 2000 },
  { id: 2, name: '专利申请 - 人脸识别', customer: '旷视科技', type: '专利', status: '已提交', applicationDate: '2026-02-01', deadlineDate: '2026-04-10', paidAmount: 6000, owedAmount: 6000 },
  { id: 3, name: '版权登记 - 软件著作权', customer: '腾讯科技', type: '版权', status: '已完成', applicationDate: '2026-01-01', deadlineDate: '2026-03-25', paidAmount: 3000, owedAmount: 0 },
  { id: 4, name: '商标续展 - 微信', customer: '腾讯科技', type: '商标', status: '进行中', applicationDate: '2026-03-01', deadlineDate: '2026-04-05', paidAmount: 2000, owedAmount: 2000 },
  { id: 5, name: '专利申请 - 人工智能', customer: '百度网讯', type: '专利', status: '准备中', applicationDate: '2026-03-15', deadlineDate: '2026-04-20', paidAmount: 0, owedAmount: 12000 }
])
...
const formData = ref({
  ...
  deadlineDate: new Date(Date.now() + 60 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
  paidAmount: 0,
  owedAmount: 0
})
...
// 打开编辑表单
const openEditForm = (caseItem: any) => {
...
const validateForm = () => {
  ...
  // 验证申请日期
  if (!formData.value.applicationDate) {
    errors.value.applicationDate = '申请日期不能为空'
    isValid = false
  }
  
  // 验证截止日期
  if (!formData.value.deadlineDate) {
    errors.value.deadlineDate = '截止日期不能为空'
    isValid = false
  } else if (new Date(formData.value.deadlineDate) < new Date(formData.value.applicationDate)) {
    errors.value.deadlineDate = '截止日期不能早于申请日期'
    isValid = false
  }
...
const submitForm = () => {
  ...
  if (isEditing.value) {
    // 编辑案件
    const index = cases.value.findIndex(c => c.id === formData.value.id)
    if (index > -1) {
      cases.value[index] = { ...formData.value }
      showMessage('案件编辑成功')
    }
  } else {
    // 添加案件
...
const handleSearch = () => {
  console.log('搜索案件:', searchQuery.value)
  // 搜索逻辑已经通过computed实现
...
const handleViewCase = (caseItem: any) => {
  ...
  showDetail('案件详情', {
    '案件名称': caseItem.name,
    '客户': caseItem.customer,
    '类型': caseItem.type,
    '状态': caseItem.status,
    '申请日期': caseItem.applicationDate,
    '截止日期': caseItem.deadlineDate
  })
}

const handleDeleteCase = (caseItem: any) => {
  console.log('删除案件:', caseItem)
  // 使用确认对话框进行删除操作
  showConfirm(`确定要删除案件 ${caseItem.name} 吗？`, () => {
